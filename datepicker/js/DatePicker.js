...
function Datepicker() {
	...
	this.regional = {
		'': { // US/English
			...
			clearText: '<span>Clear</span>', // Text for the clear command
			clearStatus: 'Clear all the dates', // Status text for the clear command
			closeText: '<span>Close</span>', // Text for the close command
...
$.extend(Datepicker.prototype, {
	...
	defaultRenderer: {
		...
		month: '<div class="datepick-month"><div class="datepick-month-header">{monthHeader}<br class="clear" /></div>' +
	...
	_attachments: function(target, inst) {
		...
		$('#calImg').remove(); // remove span class="r" and added again after it from the front end
	...
	_updateInput: function(target, keyUp) {
		...
		if (inst) {
			...
			if (onSelect && !keyUp && !inst.inSelect) {
				...
				/* redcom@gmail.com addition
				 if there is no event onSelect attached than trigger the change event for this element*/
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
		...
		if (yearRange == 'any') {
			selector = '<select class="' + this._monthYearClass + ' ' + this._anyYearClass +
				' datepick-month-year2" title="' + inst.get('yearStatus') + '">' +
...
jQuery(document).ready(function() {
	jQuery(".datepicker").each(function(id){
		var i = id+1; // make it start the id from 1 and not form 0;
		$(this).after('<span id="calImg_'+i+'" class="trigger"></span>');
		$(this).datepick({ showTrigger: '#calImg_'+i});
	});
})